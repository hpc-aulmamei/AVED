...
def start_core(mem, core_index):
    base = MMIO_OFFSETS[core_index]
    write32(mem, base + REG_AP_CTRL, 0x1)
...
def main():
    ...
    for core_index in CORES:
        ...
        print(f"[+] Starting Core {core_index}...")
        t0 = time.perf_counter_ns()
        start_core(mem, core_index)
        success = wait_for_done(mem, core_index)
        t1 = time.perf_counter_ns()

        if success:
            duration = (t1 - t0) / 1e9
            throughput = LENGTH_BYTES / duration / (1024 ** 3)  # GB/s
            total_time += duration
            total_bytes += LENGTH_BYTES
            throughputs.append(throughput)
            print(f"[✓] Core {core_index} done in {duration*1000:.3f} ms ({throughput:.2f} GB/s)")
        else:
            print(f"[✗] Core {core_index} failed.")
    ...
    print(f"Average throughput: {total_bytes / total_time / (1024 ** 3):.2f} GB/s")
